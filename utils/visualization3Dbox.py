...
def draw_birdeyes_nolabels(ax2, line_p, shape):
    # Set box color to red green blue for car cyclist pedestrian
    detected_class = line_p[0]
    if detected_class == 'Car':
        box_color = 'green'
    elif detected_class == 'Cyclist':
        box_color = 'yellow'
    elif detected_class == 'Pedestrian':
        box_color = 'cyan'
    ...
    codes[-1] = Path.CLOSEPOLY
    pth = Path(pred_corners_2d, codes)
    p = patches.PathPatch(pth, fill=False, color=box_color, label=detected_class)
    ax2.add_patch(p)
...
def visualization_nolabels(args, image_path, label_path, calib_path, pred_path,
                           dataset, VEHICLES):
    for index in range(start_frame, end_frame):
        image_file = os.path.join(image_path, dataset[index] + '.png')
        # label_file = os.path.join(label_path, dataset[index] + '.txt')
        ...
        with open(prediction_file) as f2:
            for line_p in zip(f2):
                line_p = line_p[0].strip().split(' ')
                ...
                if line_p[0] in VEHICLES and truncated < trunc_level:
                    ...
                    if line_p[0] == 'Cyclist':
                        ...
                        color = 'cyan'
                    draw_3Dbox(ax, P2, line_p, color)
                    draw_birdeyes_nolabels(ax2, line_p, shape)
        ...
        if len(handles) > 0:
            if len(handles) == 1:
                ...
                legend = ax2.legend([handles[0], handles[1], handles[2]], [labels[0], labels[1], labels[2]],
                                    loc='lower right',
                                    fontsize='x-small', framealpha=0.2)
            else:
                legend = ax2.legend([handles[0], handles[1], handles[2]], [labels[0], labels[1], labels[2]],
...
def main(args):
    base_dir = '/home/spyros/Spyros/temp_repos/SMOKE/datasets/kitti'
    dir = ReadDir(base_dir=base_dir, subset=args.a, labels=args.labels)
    ...
    # If labels are available, do standard visualisation, otherwise run the case of no labels
    if args.labels:
        visualization(args, image_path, label_path, calib_path, pred_path,
                      dataset, VEHICLES)
    else:
        visualization_nolabels(args, image_path, label_path, calib_path, pred_path,
                               dataset, VEHICLES)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '-dataset', type=str, default='testing', help='training dataset or testing')
    parser.add_argument('-s', '--save', type=bool, default=True, help='Save Figure or not')
    parser.add_argument('-p', '--path', type=str, default='/home/spyros/Spyros/temp_repos/SMOKE/outputs/output_images',
                        help='Output Image folder')
    parser.add_argument('--labels', type=str.lower, choices=['true', 'false', '1', '0'], default='true',
                        help='Enable labels (0, 1, true, false)')
    args = parser.parse_args()

    # eset labels to true or false
    labels_enabled = args.labels in ['true', '1']
    args.labels = labels_enabled
